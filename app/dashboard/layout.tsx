...
export default function DashboardLayout({
  ...
  const { t, language, setLanguage, currency, setCurrency, user } =
    useLanguage();
      ...
      try {
        ...
        if (lastMsg && lastMsg._id !== lastProcessedMsgId.current) {
          ...
          if (!isMe) {
            ...
            // --- SIGNAL: CALL START ---
            const callMatch = lastMsg.content.match(
              /\[CALL_STARTED:(audio|video)\]/,
            );
            if (callMatch) {
              ...
              if (Date.now() - msgTime < 45000) {
                ...
                ringtoneRef.current
                  .play()
                  .catch(() => console.log("Audio requires interaction"));
              }
            }

            // --- SIGNAL: STOP EVERYTHING ---
            else if (
              ["[CALL_ENDED]", "[CALL_DECLINED]", "[CALL_ACCEPTED]"].includes(
                lastMsg.content,
              )
            ) {
  ...
  const getNavItems = () => {
    ...
    const freelancerLinks = [
      ...
      // ✅ FIX: These only appear once the freelancer opts in via Settings ("canHire").
      // Previously always shown, but the API rejects job creation from non-clients anyway.
      ...(user?.canHire
        ? [
            {
              name: t.nav.postJob || "Post a Job",
              icon: <PlusLg />,
              href: "/dashboard/client/post-job",
            },
            {
              name: t.nav.jobs || "My Posted Jobs",
              icon: <ListTask />,
    ...
    const adminLinks = [
      ...
      // ✅ FIX: These pages existed with no sidebar entry.
      {
      ...
      {
        name: t.nav.contact || "Contact Messages",
        icon: <EnvelopeAt />,
  ...
  // ✅ NEW: Dedicated mobile bottom-nav set. Unlike the old slice(0,5) on
  // mainNavItems, this can never silently drop Wallet or any admin page —
  // it's a fixed 4 items + a "More" button that opens the full drawer.
  ...
  const mobileNavItems = [
    { name: t.nav.home || "Home", icon: <HouseDoor />, href: "/" },
    ...(mobilePrimaryItem ? [mobilePrimaryItem] : []),
    {
      name: t.nav.inbox,
      icon: <ChatDots />,
      href: "/dashboard/messages",
  ...
  const secondaryNavItems = [
    ...
    // ✅ FIX: This page existed with no sidebar entry.
    {
      name: t.nav.referrals || "Referrals",
      icon: <Gift />,
    ...
    <AuthGuard>
      <div className="min-h-screen bg-gray-50 flex flex-col md:flex-row font-sans">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-6 scrollbar-thin scrollbar-thumb-white/20 hover:scrollbar-thumb-white/30">
            ...
            <nav className="space-y-1.5">
              ...
              {mainNavItems.map((item) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    <span
                      className={`text-xl ${isActive(item.href) ? "" : "text-gray-400 group-hover:text-white"}`}
                    >
                      {item.icon}
                    </span>
                    {item.name}
              ...
              {secondaryNavItems.map((item) => (
                ...
                >
                  <span className="text-xl text-gray-400 group-hover:text-white">
                    {item.icon}
                  </span>
                  {item.name}
                </Link>
            ...
            <div className="relative bg-navy w-80 h-full shadow-2xl flex flex-col animate-in slide-in-from-left duration-300 border-r border-white/10">
              <div className="bg-white p-6 flex items-center justify-between shadow-sm shrink-0">
                ...
                <button
                  type="button"
                  onClick={() => setIsMobileMenuOpen(false)}
                  className="bg-gray-100 text-gray-500 hover:bg-red-100 hover:text-red-500 p-1.5 rounded-full transition-colors"
                >
                  <X className="text-xl" />
                </button>
              </div>
                  ...
                  <nav className="space-y-1">
                    {secondaryNavItems.map((item) => (
                      <Link
                        ...
                        href={item.href}
                        onClick={() => setIsMobileMenuOpen(false)}
                        className="flex items-center gap-4 px-3 py-3.5 rounded-xl transition-all active:scale-95 text-gray-300 hover:text-white hover:bg-white/10"
                      >
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="w-10 h-10 bg-gray-200 rounded-full overflow-hidden border border-gray-100 relative">
                  {user?.avatar ? (
                    ...
                    <Person className="text-2xl text-gray-400 m-2" />
                  )}
                </div>
              </div>
            </div>
        ...
        <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 flex justify-around py-3 z-50 pb-safe shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
          {mobileNavItems.map((item) => {
            ...
            return (
              <Link
                key={item.name}
                href={item.href}
                className={`flex flex-col items-center gap-1 transition-colors relative ${active ? "text-navy font-bold" : "text-gray-400 hover:text-navy"}`}
              >
          ...
          {/* ✅ NEW: "More" opens the existing full drawer — guarantees every
            sidebar link is reachable on mobile, no matter how many a role has */}
          <button
            onClick={() => setIsMobileMenuOpen(true)}
            className="flex flex-col items-center gap-1 transition-colors text-gray-400 hover:text-navy"
          >
            <List className="text-xl" />
            <span className="text-[10px]">{t.nav.menu || "More"}</span>
          </button>
        </nav>
      </div>
            ...
            <div className="flex flex-col items-center gap-4">
              <div className="relative">
                <div className="w-32 h-32 rounded-full border-4 border-white/10 overflow-hidden shadow-2xl relative z-10 bg-gray-800">
                  {incomingCall.contact?.avatar ? (
                    <Image
                      src={incomingCall.contact.avatar}
                      alt="Caller"
                      fill
                      unoptimized
                      className="object-cover"
              ...
              <div className="text-center">
                <h3 className="text-3xl font-bold text-white tracking-tight">
                  {incomingCall.contact?.name || "Unknown Caller"}
                </h3>
                <p className="text-blue-200/80 font-medium text-sm mt-1 uppercase tracking-widest flex items-center justify-center gap-2">
                  {incomingCall.type === "video" ? (
                    <CameraVideo className="animate-pulse" />
                  ) : (
                    <Telephone className="animate-pulse" />
                  )}
