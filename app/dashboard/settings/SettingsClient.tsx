...
import { 
    Globe, ShieldLock, Bell, Save, CheckCircleFill, 
    PersonBadge, Eye, Phone, Envelope, PlayBtn, Lightning, ToggleOn, ToggleOff,
    Moon, Sun, Trash, ExclamationTriangle, CloudUpload, XCircleFill, ChevronDown,
    ExclamationTriangleFill, QrCodeScan 
...
export default function SettingsContent() {
  ...
  const fetchSettings = async () => {
  ...
  }, [user, formData, refreshUser, setLanguage, t.settings.success, t.settings.errorSave]);

    const handleRequestVerification = useCallback(async () => {
    if (idFiles.length === 0 && (!formData.identityDocuments || formData.identityDocuments.length === 0)) {
    ...
    }, [idFiles, formData, docType, user._id, t.settings.errorNoDoc, t.settings.verificationSent, t.workspace.errorUpload]);
  ...
  const handle2FASetup = async () => {
      ...
      try {
          ...
          console.error(error);
          setFeedback({ type: "error", message: t.settings.errorSave || "Connection failed" });
      } finally {
          setLoading(false);
      }
  };
  ...
  const CustomSelect = ({ label, value, onChange, options, disabled = false }: any) => {
      const id = useId();
      return (
  ...
  const tabs = [
      ...
      { id: "data", label: t.settings.dataPrivacy, icon: <ExclamationTriangle /> },
  ];
    ...
    <div className="max-w-5xl mx-auto pb-20">
      ...
      <div className="flex flex-col md:flex-row gap-6">
          ...
          <div className="flex-1 bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                  ...
                  <div className="space-y-6">
                      ...
                      <div className="grid md:grid-cols-2 gap-6">
                          ...
                          <div>
                              ...
                              <div className="flex bg-gray-50 p-1 rounded-xl border border-gray-200" role="group">
                                {["light", "dark", "system"].map(tv => (
                                    <button 
                                      ...
                                      className={`flex-1 py-2 rounded-lg text-xs font-bold capitalize transition-all ${
                                          ...
                                          formData.theme === tv ? "bg-white text-navy shadow-sm" : "text-gray-400"
                                      }`}
                                    >
                                        {getThemeIcon(tv)}
                                        {tv}
                      ...
                      <div className="bg-blue-50 p-6 rounded-xl border border-blue-100">
                          <div className="flex justify-between items-start mb-6">
                              <div>
                                  <h4 className="font-bold text-navy text-sm">{t.settings.idVerify}</h4>
                                  <p className="text-xs text-gray-500 mt-1">
                                      {t.manage.status}: <span className="font-bold uppercase">{formData.verificationStatus}</span>
                                  </p>
                              </div>
                              
                              {formData.verificationStatus === "pending" && (
                                  <span className="text-xs font-bold text-orange-500 bg-white border border-orange-200 px-3 py-1 rounded-full">{t.workspace.pendingReview}</span>
                              )}
                              
                              {(formData.verificationStatus === "verified" || user?.isVerified) && (
                          ...
                          {formData.verificationStatus !== "verified" && formData.verificationStatus !== "pending" && !user?.isVerified && (
                              <div className="space-y-5 animate-in fade-in slide-in-from-top-2">
                                  ...
                                  {formData.verificationStatus === "rejected" && (
                                      <div className="bg-red-50 text-red-800 text-xs p-3 rounded-lg border border-red-100 flex items-start gap-2">
                                          ...
                                          <p><strong>{t.settings.verifyFailed}:</strong> {t.settings.verifyFailDesc}</p>
                                      </div>
                                  )}

                                  <div className="bg-blue-100 text-blue-800 text-xs p-3 rounded-lg border border-blue-200 flex items-start gap-2">
                                      <ExclamationTriangle className="text-sm mt-0.5 flex-shrink-0" />
                                      <p><strong>Important:</strong> {t.settings.verifyTip}</p>
                                  </div>
                                  ...
                                  <button type="button" onClick={() => idInputRef.current?.click()} className="w-full p-6 bg-white border-2 border-dashed rounded-xl text-center hover:bg-blue-50/50 border-blue-200 transition-colors flex flex-col items-center justify-center gap-2">
                                      <CloudUpload className="text-3xl text-blue-300" />
                                      <div className="text-center">
                                          <span className="text-xs font-bold text-navy block">{t.workspace.uploadFiles}</span>
                                          <span className="text-[10px] text-gray-400">{t.settings.frontBack}</span>
                                      </div>
                                      {idFiles.length > 0 ? (
                                          <div className="flex flex-wrap gap-2 justify-center mt-2">
                                              {idFiles.map((f, i) => (
                                                  <span key={i} className="text-[10px] bg-gray-100 px-2 py-1 rounded text-gray-600 border border-gray-200">{f.name}</span>
                                              ))}
                                          </div>
                                      ) : formData.identityDocuments?.length > 0 && (
                                          <span className="text-xs text-green-600 font-bold mt-1"><CheckCircleFill className="inline mr-1" /> {t.settings.docsOnFile}</span>
                                      )}
                                      <input type="file" multiple ref={idInputRef} className="hidden" accept="image/*,.pdf" onChange={(e) => e.target.files && setIdFiles(Array.from(e.target.files))} />
                                  </button>

                                  <button onClick={handleRequestVerification} disabled={saving} className="w-full py-3 bg-navy text-white text-xs font-bold rounded-xl hover:bg-navy-light shadow-md transition-all disabled:opacity-50">
              ...
              {activeTab === "general" && (
                  <div className="space-y-6">
                      <h3 className="text-lg font-bold text-navy border-b border-gray-100 pb-2">{t.nav.general} {t.settings.preferences}</h3>
                      ...
                      <div className="grid md:grid-cols-2 gap-6">
                          ...
                          <CustomSelect 
                              ...
                              onChange={(e: any) => setFormData({...formData, contentLanguage: e.target.value})}
                              options={[
                                  { value: "en", label: "English" },
                                  { value: "fr", label: "Français" },
                      ...
                      <div className="space-y-4">
                          <div className="flex items-center justify-between">
                              <span className="text-sm font-medium text-navy flex items-center gap-2"><PlayBtn /> {t.settings.autoplay}</span>
                              <Toggle checked={formData.autoplayVideo} onChange={v => setFormData({...formData, autoplayVideo: v})} />
                          </div>
                          <div className="flex items-center justify-between">
                              <span className="text-sm font-medium text-navy flex items-center gap-2"><Lightning /> {t.settings.reduceAnim}</span>
                              <Toggle checked={formData.reduceAnimations} onChange={v => setFormData({...formData, reduceAnimations: v})} />
                          </div>
                      </div>

                      <div className="pt-6 border-t border-gray-100">
                          <h4 className="text-red-600 font-bold text-sm mb-2">{t.settings.dangerZone}</h4>
                          <button 
              ...
              {activeTab === "security" && (
                  <div className="space-y-6">
                      ...
                      <div>
                          <label htmlFor="passwordInput" className="block text-xs font-bold text-gray-500 uppercase mb-1">{t.settings.password}</label>
                          <input 
                            id="passwordInput"
                            type="password"
                      ...
                      <div className="bg-gray-50 p-4 rounded-xl border border-gray-200">
                          <div className="flex items-center justify-between">
                              <div>
                                  <h4 className="font-bold text-navy text-sm">{t.settings.twoFa}</h4>
                                  <p className="text-xs text-gray-500">{t.settings.twoFaDesc}</p>
                              </div>
                              {formData.twoFactorEnabled ? (
                                  <span className="text-green-600 font-bold text-xs flex items-center gap-1"><CheckCircleFill /> {t.settings.enabled}</span>
                              ) : (
                                  <button 
              ...
              {activeTab === "notifications" && (
                  <div className="space-y-6">
                      <h3 className="text-lg font-bold text-navy border-b border-gray-100 pb-2">{t.notificationsPage.title}</h3>
                      <div className="space-y-2">
                          {[
                              { id: "push", label: t.settings.pushNotif, icon: <Bell /> },
                              { id: "inApp", label: t.settings.inAppNotif, icon: <Lightning /> },
                              { id: "email", label: t.settings.emailNotif, icon: <Envelope /> },
                              { id: "sms", label: t.settings.smsNotif, icon: <Phone /> },
                          ].map(opt => (
                              <div key={opt.id} className="flex items-center justify-between p-3 border border-gray-100 rounded-xl hover:bg-gray-50 transition-colors">
                                  <span className="text-sm font-medium text-navy flex items-center gap-3 text-gray-600">{opt.icon} {opt.label}</span>
                                  <Toggle 
                                    checked={formData.notifications[opt.id as keyof typeof formData.notifications]} 
                                    onChange={v => setFormData({
                                        ...formData, 
                                        notifications: { ...formData.notifications, [opt.id]: v }
                                    })} 
                                  />
                              </div>
                          ))}
              ...
              {activeTab === "visibility" && (
                  <div className="space-y-6">
                      <h3 className="text-lg font-bold text-navy border-b border-gray-100 pb-2">{t.settings.visibility}</h3>
                      
                      <div>
                          <span className="block text-xs font-bold text-gray-500 uppercase mb-2">{t.settings.profilePrivacy}</span>
                          <div className="flex flex-col gap-2">
                              {["public", "clients_only", "private"].map(vis => (
                                  <label key={vis} className="flex items-center gap-3 p-3 border border-gray-200 rounded-xl cursor-pointer hover:bg-gray-50 transition-colors">
                                      <input 
                                        type="radio" 
                                        name="visibility" 
                                        checked={formData.profileVisibility === vis}
                                        onChange={() => setFormData({...formData, profileVisibility: vis})}
                                        className="text-navy focus:ring-navy accent-navy"
                                      />
                                      <span className="capitalize text-sm font-medium text-navy">{vis.replace("_", " ")}</span>
                                  </label>
                              ))}
                          </div>
                      </div>
                      ...
                      <div className="flex items-center justify-between pt-4">
                          <span className="text-sm font-medium text-navy">{t.settings.onlineStatus}</span>
                          <Toggle checked={formData.showOnlineStatus} onChange={v => setFormData({...formData, showOnlineStatus: v})} />
                      </div>
                  </div>
              )}
              ...
              {activeTab === "data" && (
                  <div className="space-y-6">
                      <h3 className="text-lg font-bold text-navy border-b border-gray-100 pb-2">{t.settings.dataPrivacy}</h3>
                      
                      <div className="bg-gray-50 p-4 rounded-xl border border-gray-200">
                          <h4 className="font-bold text-navy text-sm mb-2">{t.settings.dataDownload}</h4>
                          <p className="text-xs text-gray-500 mb-3">
                              {t.settings.dataDownloadDesc}
                          </p>
                          <button onClick={() => setFeedback({type:"success", message: t.settings.archiveRequested})} className="px-4 py-2 bg-white border border-gray-300 rounded-lg text-xs font-bold text-navy hover:bg-gray-100">
                      ...
                      <div className="space-y-4">
                          <div className="flex items-center justify-between">
                              <div>
                                  <span className="text-sm font-medium text-navy block">{t.settings.allowData}</span>
                                  <span className="text-xs text-gray-400">{t.settings.allowDataDesc}</span>
                              </div>
                              <Toggle checked={formData.allowDataCollection} onChange={v => setFormData({...formData, allowDataCollection: v})} />
                          </div>
                          
                          <div className="flex items-center justify-between">
                              <div>
                                  <span className="text-sm font-medium text-navy block">{t.settings.searchVis}</span>
                                  <span className="text-xs text-gray-400">{t.settings.searchVisDesc}</span>
                              </div>
                              <Toggle checked={formData.profileVisibility === "public"} onChange={v => setFormData({...formData, profileVisibility: v ? "public" : "private"})} />
                          </div>
                      </div>
                  </div>
              )}
              ...
              <div className="mt-8 pt-6 border-t border-gray-100 flex gap-4">
                  ...
                  >
                      {saving ? t.proposal.submitting : <><Save /> {t.settings.save}</>}
                  </button>
                  <button 
      ...
      {showDeactivateModal && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-in fade-in">
              <div className="bg-white w-full max-w-sm rounded-2xl p-6 shadow-2xl text-center">
                  ...
                  <h3 className="font-bold text-navy text-lg mb-2">{t.settings.deactivateTitle}</h3>
                  <p className="text-sm text-gray-500 mb-6">
                      {t.settings.deactivateDesc}
                      <br/><br/>
                      {t.settings.typeDelete} <strong>DELETE</strong> {t.settings.toConfirm}.
                  </p>
      ...
      {show2FAModal && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-in fade-in">
              <div className="bg-white w-full max-w-sm rounded-2xl p-6 shadow-2xl text-center">
                  <h3 className="font-bold text-navy text-lg mb-4">{t.settings.scanQr}</h3>
                  ...
                  <div className="mb-6">
                      ...
                      <input 
                          type="text" 
                          value={verifyCode}
                          onChange={(e) => setVerifyCode(e.target.value.replace(/\D/g, '').slice(0, 6))} // Only numbers, max 6
                          placeholder="123 456"
                  ...
                  <div className="flex gap-3">
                      <button 
                          onClick={() => { setShow2FAModal(false); setVerifyCode(""); }} 
                          className="flex-1 py-3 bg-gray-100 text-gray-600 font-bold rounded-xl hover:bg-gray-200 transition-colors"
                      >
                          {t.proposal.cancel}
                      </button>
                      ...
                      >
                          {saving && <span className="w-3 h-3 border-2 border-white/50 border-t-white rounded-full animate-spin"></span>}
